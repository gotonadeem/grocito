    ...
    <div class="filter-sidebar">
        <div class="categori-sec mb-3">
            <h4>Categories</h4>
            <ul class="nav flex-column">
                <li class="nav-item"> <a class="nav-link" href="{{URL::to('category/'.$catData->slug)}}">{{$catData->name}}</a>
                    <ul class="nav flex-column sub-menu">
					    @foreach($category_filter as $vs)
                        <li class="nav-item"> <a class="nav-link" href="{{URL::to('category/'.$catData->slug.'/'.$vs->slug)}}">{{$vs->name}}</a> </li>
                        @endforeach
                    </ul>
				...
				<li class="nav-item">
                    <a class="nav-link" href="#">
                        <div class="custom-control custom-checkbox">
                            <input onclick="filter_now()" type="checkbox" class="custom-control-input brand_value" value="{{$vs['brand']['id']}}" id="FruitsDrinks{{$ks}}" name="brand_value">
                            <label class="custom-control-label" for="FruitsDrinks{{$ks}}">{{$vs['brand']['name']}}</label>
                        </div>
                    </a>
                </li>
               @endforeach
